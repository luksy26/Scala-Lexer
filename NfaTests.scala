class NfaTests extends munit.FunSuite {
  ...
  test("my tests") {
    ...
    val tree: Nfa.regexTree = Nfa.getTree("PLUS UNION a STAR MAYBE b")
    Nfa.printTree(tree)
    val nfa: Nfa[Int] = Nfa.nfaFromTree(tree)
    ...
    print(nfa.Nr_Stari)
    print(" stari\nStare initiala: ")
    print(nfa.Stare_Initiala)
    print("\nStari finale: ")
    print(nfa.Stari_Finale)
    print("\nTranzitii: ")
    print(nfa.Tranzitii)
    print("\n")
  }
  ...
  test("Nfa map") {
    // this is a class used just for this test
		// its get_mapping method receives a 'state' and returns a unique id for it. if it receives the same state multiple time it returns the same id
    class TestIterator[A](start: Int) {
      var counter: Int = start
      var mapping: Map[A, Int] = Map[A, Int]()
    ...
    assert(regexes.forall((p: (String, String, Int => Any)) => { // run test for each tuple in "regexes"
        ...
        // (epsilon-transitions are not checked)
          alphabet.forall(c =>
            states.forall(s =>
