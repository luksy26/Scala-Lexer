class RegexParseTests extends munit.FunSuite {
  ...
  test("chec") {
    ...
    val s = "\' \'\'a\'"
    Regex.printEither1(Regex.preprocess(s.toList))
    Regex.printEither2(Regex.preprocess(s.toList))


    val s1 = "a([a-z]*|[A-Z]*)z"
    val s2 = Regex.preprocess(s1.toList)
    Regex.printEither1(s2)
    val tree: Nfa.regexTree = Regex.treeFromRegex(s2)
    Nfa.printTree(tree)
    println(Nfa.treeToPrenex(tree))


    val dfa: Dfa[Int] = Dfa.fromPrenex(Regex.toPrenex(s1))
    print("alfabet : ")
    print(dfa.Alfabet.toList.sorted)
    print("\n")
    print(dfa.Nr_Stari)
    print(" stari: ")
    println(dfa.Stari.toList.sorted)
    print("Stare initiala: ")
    println(dfa.Stare_Initiala)
    print("\nStari finale: ")
    print(dfa.Stari_Finale.toList.sorted)
    print("\nTranzitii: ")
    print(dfa.Tranzitii.toList.sorted)
    print("\n")
    println(dfa.accepts("abz"))
  }
